...
class OKXAdapter(ExchangeAdapter):
    ...
    async def _stream_messages(self) -> AsyncGenerator[dict[str, Any], None]:
        ...
        async with websockets.connect(self._BASE_WSS_URL) as websocket:
            ...
            logger.info(f"[{self.venue_name}] Subscribed to 'trades' for: {[a['instId'] for a in args]}")
            ...
            while True:
                ...
                try:
                    ...
                    logger.warning(f"[{self.venue_name}] WebSocket timeout. Sending keepalive ping.")
    ...
    def _normalize_message(self, message: dict[str, Any]) -> models_pb2.PriceUpdate | None:
        """
        Normalizes a raw trade data object from OKX into a canonical PriceUpdate.
        """
        try:
    ...
    async def get_historical_candles(
        ...
        """
        Fetches historical OHLCV data from OKX's REST API, handling pagination.
        """
        ...
        while current_end_ms > start_ms:
            ...
            try:
                ...
                for c in candles_data:
                    ...
                    all_candles.append(
                        models_pb2.Candle(
                            ...
                            # OKX doesn't provide close time, so we calculate it
                            close_time=normalize_timestamp_to_rfc3339(open_time + timedelta(seconds=self._map_bar_to_seconds(bar))),
                            open=str(c[1]),
                            high=str(c[2]),
                            low=str(c[3]),
                            close=str(c[4]),
                ...
                # Paginate backwards by setting the new 'after' to the timestamp of the oldest candle received
