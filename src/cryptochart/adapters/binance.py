...
class BinanceAdapter(ExchangeAdapter):
    ...
    def _normalize_symbol_to_venue(self, symbol: str) -> str:
        """Converts a 'BASE/QUOTE' symbol to Binance's 'BASEQUOTE' format (lowercase)."""
    ...
    def _normalize_symbol_from_venue(self, venue_symbol: str) -> str:
        ...
        # This is a simple heuristic. A robust solution would use the /exchangeInfo endpoint.
    ...
    async def _stream_messages(self) -> AsyncGenerator[dict[str, Any], None]:
        """
        Connects to the WebSocket, subscribes to trade streams, and yields messages.
        """
        streams = [
    ...
    def _normalize_message(self, message: dict[str, Any]) -> models_pb2.PriceUpdate | None:
        """
        Normalizes a raw WebSocket message from Binance into a canonical PriceUpdate.
        """
        if message.get("e") != "trade":
            return None

        try:
            # If 'm' is True, the buyer is the maker, which means it was a sell-side initiated trade.
    ...
    async def get_historical_candles(
        ...
        """
        Fetches historical OHLCV data from Binance's REST API, handling pagination.
        """
        ...
        while current_start_ms < end_ms:
            ...
            try:
                response = await self.http_client.get(f"{self._BASE_API_URL}/klines", params=params)
