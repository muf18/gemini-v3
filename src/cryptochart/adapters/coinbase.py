...
class CoinbaseAdapter(ExchangeAdapter):
    ...
    async def _stream_messages(self) -> AsyncGenerator[dict[str, Any], None]:
        """
        Connects to the WebSocket, subscribes to the 'matches' channel,
        ...
        async with websockets.connect(self._BASE_WSS_URL) as websocket:
            ...
            logger.info(f"[{self.venue_name}] Subscribed to 'matches' for: {venue_symbols}")
    ...
    def _normalize_message(self, message: dict[str, Any]) -> models_pb2.PriceUpdate | None:
        """
        Normalizes a raw WebSocket message from Coinbase into a canonical PriceUpdate.
        """
        ...
        if msg_type == "match" or msg_type == "last_match":
            try:
                ...
                logger.warning(f"[{self.venue_name}] Could not parse match message: {message}. Error: {e}")
                return None
    ...
    async def get_historical_candles(
        ...
        """
        Fetches historical OHLCV data from Coinbase's REST API, handling pagination.
        """
        ...
        while current_end_dt > start_dt:
            ...
            try:
                response = await self.http_client.get(
                    f"{self._BASE_API_URL}/products/{venue_symbol}/candles", params=params
                )
                ...
                for c in data:
                    ...
                    all_candles.append(
                        models_pb2.Candle(
                            ...
                            close_time=normalize_timestamp_to_rfc3339(open_time + timedelta(seconds=granularity)),
                            open=str(c[3]),
                            high=str(c[2]),
                            low=str(c[1]),
                            close=str(c[4]),
                ...
                current_end_dt = datetime.fromtimestamp(oldest_ts_in_batch, tz=timezone.utc)
